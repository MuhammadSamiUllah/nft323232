...
const sliderInit = function (currentSlider) {
  ...
  currentSlider.querySelector("[data-slider-container]");
  const sliderPrevBtn =
   currentSlider.querySelector("[data-slider-prev]");
  const sliderNextBtn =
   currentSlider.querySelector("[data-slider-next]");

  const totalSliderVisibleItems =
Number(getComputedStyle(currentSlider)
.getPropertyValue("--slider-item"));
  const totalSliderItems = 
  sliderContainer.childElementCount -
...
`translateX(-${sliderContainer.
  children[currentSlidePos].offsetLeft}px)`;
  }
  ...
  const startAutoSlide = () => autoSlideInterval = 
...
const accordionInit = function (currentAccordion) {
  ...
  const accordionBtn =
 currentAccordion.querySelector("[data-accordion-btn]");
